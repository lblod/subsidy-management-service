...
import { SERVICE_URI } from '../../env.js';
import { filenameToUri, uriToFilename } from '../util/file.js';
...
export class SemanticFile {
  ...
  get virtualUUID() {
    return uuidv5(this.uuid, NAMESPACE);
  }

  get virtualURI() {
    return `http://data.lblod.info/files/${this.virtualUUID}`;
  }
  ...
  toNT() {
    ...
    const escaped = {
      ...
      virtualUUID: sparqlEscape(this.virtualUUID, 'string'),
      physicalURI: `<${this.physicalURI}>`,
      physicalUUID: sparqlEscape(this.uuid, 'string'),
    ...
    buffer.push(`${escaped.virtualURI} mu:uuid ${escaped.virtualUUID} .`);
    ...
    buffer.push(`${escaped.physicalURI} mu:uuid ${escaped.physicalUUID} .`);
